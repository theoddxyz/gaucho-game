...
export class IsoControls {
  constructor(camera) {
    ...
    this._working    = false;   // true while trabajando animation plays — blocks movement+rotation
  ...
  update(dt, colliders, speedMult = 1.0, onHorse = false) {
    ...
    const dir = new THREE.Vector3();
    if (!this._stunned && !this._sleeping && !this._working) {
      if (this.keys.w) dir.z -= 1;
      if (this.keys.s) dir.z += 1;
      if (this.keys.a) dir.x -= 1;
      if (this.keys.d) dir.x += 1;
    ...
    if (onHorse) {
      ...
      const accel = dir.length() > 0 ? 5 : (this._stunned ? 0.8 : 2);   // frenado suave; más lento en stun (preserva knockback)
