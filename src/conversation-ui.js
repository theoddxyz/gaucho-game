...
export class ConversationUI {
  constructor() {
    this._panel          = null;
    this._active         = false;
    this._current        = null;   // { name, intention, cuadrante, trayectoria, energia, recursos, vecinos }
    this._qaCache        = {};     // name → [{q, a}]
    this._history        = {};     // name → [{from:'player'|'npc', text}]  (sesión)
    this._customUsed     = {};     // name → gameDay
    this._currentGameDay = 1;
    this._waiting        = false;
    this.onClose         = null;   // callback para main.js
  ...
  setGameDay(day) {
    if (day !== this._currentGameDay) {
      ...
      this._customUsed = {};   // resetear límite de mensajes custom
    }
  }

  // Pedir Q&A diaria al servidor (al joinear y al cambiar de día)
  ...
  _buildPanel() {
    ...
    el.innerHTML = `
      ...
      <div id="conv-history" style="
        padding: 8px 16px 4px;
        min-height: 36px; max-height: 150px;
        overflow-y: auto;
        font-size: 13px; line-height: 1.55;
      "></div>
      <div id="conv-questions" style="
  ...
  _renderHeader() {
    ...
    document.getElementById('conv-name').textContent = a.name;
    document.getElementById('conv-mood').textContent = MOOD_LABEL[a.intention] || '';
  }
  ...
  _renderQuestions() {
    ...
    // Botones de preguntas pre-armadas
    ...
    const customBtn = _mkBtn(
      usedToday ? '(ya le dijiste algo hoy)' : 'Decirle algo...',
      usedToday
    );
    if (!usedToday) {
  ...
  _openCustomInput() {
    ...
    const sendBtn = _mkBtn('Mandar', false, `
      background: rgba(90, 55, 18, 0.8);
      margin-top: 4px;
    `);
